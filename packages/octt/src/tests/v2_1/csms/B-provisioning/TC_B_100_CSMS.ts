...
export const TC_B_100_CSMS: TestCase = {
  id: 'TC_B_100_CSMS',
  ...
  sut: 'csms',
  description:
    'The CSMS updates the connection details on the Charging Station including identity and password.',
  purpose:
    'To verify if the CSMS is able to set a new network connection profile that contains basicAuthPassword and identity.',
  execute: async (ctx) => {
    ...
    if (ctx.triggerCommand != null) {
      await ctx.triggerCommand('v21', 'SetNetworkProfile', {
        ...
        connectionData: {
          ...
          identity: 'OCTT-Virtual',
          basicAuthPassword: 'TestPassword123',
        },
      });
    } else {
    ...
    steps.push({
      step: 1,
      description: 'CSMS sends SetNetworkProfileRequest with identity and password',
    ...
    steps.push({
      ...
      status: hasConnectionData
        ? ('passed' as 'passed' | 'failed')
        : ('failed' as 'passed' | 'failed'),
      expected: 'connectionData present with identity and password fields',
