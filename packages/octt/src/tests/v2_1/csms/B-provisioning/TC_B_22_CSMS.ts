...
export const TC_B_22_CSMS: TestCase = {
  id: 'TC_B_22_CSMS',
  ...
  sut: 'csms',
  description:
    'This test case covers how the CSMS can remotely request the Charging Station to reset itself by sending a ResetRequest with type Immediate while a transaction is ongoing.',
  purpose:
  ...
  execute: async (ctx) => {
    ...
    ctx.client.setIncomingCallHandler(
      async (_messageId: string, action: string, payload: Record<string, unknown>) => {
        if (action === 'Reset') {
          ...
          // Respond Accepted, then immediately end transaction and reboot
          setTimeout(async () => {
            try {
              await ctx.client.sendCall('TransactionEvent', {
                eventType: 'Ended',
    ...
    steps.push({
      step: 1,
      description: 'CSMS sends ResetRequest with type Immediate',
    ...
    steps.push({
      step: 3,
      description: 'Reset type is Immediate',
      status:
        resetType === 'Immediate'
          ? ('passed' as 'passed' | 'failed')
          : ('failed' as 'passed' | 'failed'),
      expected: 'type = Immediate',
