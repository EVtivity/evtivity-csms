...
export const TC_B_12_CSMS: TestCase = {
  id: 'TC_B_12_CSMS',
  name: 'Get Base Report - ConfigurationInventory',
  ...
  execute: async (ctx) => {
    ...
    ctx.client.setIncomingCallHandler(
      async (_messageId: string, action: string, payload: Record<string, unknown>) => {
        if (action === 'GetBaseReport') {
          ...
          if (reportBase === 'ConfigurationInventory') {
    ...
    steps.push({
      step: 2,
      description: 'reportBase is ConfigurationInventory',
      status: reportBaseCorrect
        ? ('passed' as 'passed' | 'failed')
        : ('failed' as 'passed' | 'failed'),
      expected: 'reportBase = ConfigurationInventory',
      actual: reportBaseCorrect ? 'reportBase = ConfigurationInventory' : 'Incorrect reportBase',
    });
