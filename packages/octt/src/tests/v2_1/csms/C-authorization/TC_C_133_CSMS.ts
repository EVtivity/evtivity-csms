...
export const TC_C_133_CSMS: TestCase = {
  ...
  execute: async (ctx) => {
    ...
    // The QR code has an invalid TOTP
    ...
    // Note: This test verifies CSMS does NOT send RequestStartTransaction for invalid TOTP.
    // No triggerCommand needed - we are testing the absence of a command.
    await new Promise((resolve) => setTimeout(resolve, 5000));

    steps.push({
      step: 2,
      description: 'Verify CSMS does NOT send RequestStartTransaction for invalid TOTP',
