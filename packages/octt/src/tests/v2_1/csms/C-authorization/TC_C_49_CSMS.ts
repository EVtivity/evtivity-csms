...
export const TC_C_49_CSMS: TestCase = {
  ...
  sut: 'csms',
  description:
    'This test case covers how somebody with a Master Pass can stop all ongoing transactions without UI.',
  purpose:
    'To verify if the CSMS is able to correctly respond on a request to stop all transactions when an idToken with MasterPass GroupId is used without UI.',
  execute: async (ctx) => {
    ...
    // Step 2: Start two transactions on EVSE 1 and EVSE 2
    ...
    // Step 4: Send TransactionEvent Ended for both EVSEs with valid idToken (not MasterPass)
    // Without UI: the station uses the original idToken, not the MasterPass token
