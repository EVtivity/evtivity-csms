...
export function SmartChargingPushDetail(): React.JSX.Element {
  ...
  const { data: push, isLoading } = useQuery({
    queryKey: ['smart-charging-pushes', pushId, page],
    queryFn: () =>
      api.get<PushDetail>(
        `/v1/smart-charging/pushes/${pushId ?? ''}?page=${String(page)}&limit=${String(limit)}`,
      ),
    enabled: pushId != null,
  ...
  return (
    <div className="px-4 py-4 md:px-6 md:py-6 space-y-6">
      <div className="flex items-center gap-4">
        <BackButton to={`/smart-charging/${push.templateId}`} />
        <div>
          <h1 className="text-2xl md:text-3xl font-bold">{t('smartCharging.pushProgress')}</h1>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('smartCharging.pushProgress')}</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex gap-6 mb-4 text-sm">
            ...
            <div>
              <span className="text-muted-foreground">{t('smartCharging.accepted')}: </span>
              <span className="font-medium">{accepted}</span>
            </div>
            {rejected > 0 && (
              <div>
                <span className="text-muted-foreground">{t('smartCharging.rejected')}: </span>
                <span className="font-medium text-warning">{rejected}</span>
              </div>
            )}
            {failed > 0 && (
              <div>
                <span className="text-muted-foreground">{t('smartCharging.failed')}: </span>
