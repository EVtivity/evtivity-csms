...
export function StationAssignPricing(): React.JSX.Element {
  ...
  const assignMutation = useMutation({
    mutationFn: (pricingGroupId: string) =>
      api.post(`/v1/stations/${id ?? ''}/pricing-groups`, { pricingGroupId }),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['stations', id, 'pricing-group'] });
      void navigate(`/stations/${id ?? ''}?tab=pricing`);
    },
    onSettled: () => {
      setMutatingId(null);
    },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        <BackButton to={`/stations/${id ?? ''}?tab=pricing`} />
        <h1 className="text-2xl font-bold md:text-3xl">{t('stations.assignPricingGroup')}</h1>
      </div>
          ...
          <div className="space-y-1">
            {pricingGroups != null && pricingGroups.length > 0 ? (
              ...
              <p className="px-3 py-2 text-sm text-muted-foreground">
                {t('stations.noPricingGroupResults')}
              </p>
            )}
          </div>
        </CardContent>
      ...
      <ConfirmDialog
        ...
        variant="default"
        title={t('stations.assignPricingGroup')}
        description={t('stations.confirmAssignPricing', { name: pendingGroup?.name ?? '' })}
